...
class UnnamedOperations:  # pylint: disable=docstring-missing-param
    ...
    async def receive(self, **kwargs: Any) -> AsyncStream[_models2.Info]:
        ...
        deserialized: AsyncStream[_models2.Info] = AsyncStream(response=response, deserialization_callback=_callback, last_event_id=_last_event_id, reconnect_callback=_reconnect)  # type: ignore
        if cls:
